...
describe('messaging', () => {
  ...
  it('throws ErrMessageValidation for an invalid outbound payload', async () => {
    ...
    await expect(built.container.get<MessageBus>(MessageBus).send('emit2', { id: 'bad' }))
      .rejects.toBeInstanceOf(ErrMessageValidation)
    await built.close()
  })
  ...
  it('fires onError + recoverer when retries are exhausted', async () => {
    ...
    const app = createApplication({}).extend(messaging, m => m
      ...
      .in('orders8', { destination: 'orders', via: 'primary', retry: { attempts: 2, backoff: { type: 'fixed', delay: 1 } } }))
    const built = app.build()
    await built.run()
