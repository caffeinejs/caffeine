...
describe('CaffeineIoC', function () {
  ...
  it('should support iteration via entries() and direct Symbol.iterator', function () {
    ...
    expect(fromEntries.has(Test)).toBeTruthy()
    expect(fromEntries.has(NamedTest)).toBeTruthy()
    expect(fromIterator.has(NamedTest)).toBeTruthy()
    expect(fromEntries.get(NamedTest)?.names).toContain(kTestName)
    expect(fromEntries.size).toEqual(di.size)
    expect(fromIterator.size).toEqual(di.size)
  })
