...
describe('Manual Binding', function () {
  describe('general bindings', function () {
    ...
    const sy = token<string>(Symbol('test'))
    ...
    it('should bind to class', async function () {
      ...
      di.bind(Late, t => t.toClass(Late).lifetime(Scopes.SINGLETON))
      await di.init()
    ...
    it('should bind to class by name', async function () {
      ...
      di.bind(token<Late>('test'), t => t.toClass(Late))
      await di.init()

      const r = di.get(token<Late>('test'))
    ...
    it('should bind abstract class to concrete implementation', async function () {
      ...
      di.bind(Abs, t => t.toClass(Impl).lifetime(Scopes.SINGLETON))
      await di.init()
    ...
    it('should bind value', async function () {
      ...
      di.bind(token<string>('val'), t => t.toValue('test'))
      di.bind(StrValue, t => t.toSelf([token<string>('val')]))
      await di.init()

      const r = di.get(StrValue)
      const v = di.get(token<string>('val'))
    ...
    it('should bind factory', async function () {
      ...
      di.bind(token<string>('val'), t => t.toValue('test'))
      di.bind(sy, t => t.toFactory(({ container }) => `factory-${container.get(token<string>('val'))}`))
      di.bind(FromFactory, t => t.toSelf([sy]))
      await di.init()
    ...
    describe('toClass() with injections', function () {
      ...
      it('should resolve when multiple injections are provided', async function () {
        ...
        di.bind(TwoParam, t => t.toClass(TwoParam, [DepA, DepB]))
        await di.init()

        const svc = di.get(TwoParam)

        expect(svc.a).toBeInstanceOf(DepA)
        expect(svc.b).toBeInstanceOf(DepB)
      })
      ...
      it('should resolve when a single injection is provided without wrapping array', async function () {
        ...
        di.bind(DepA, t => t.toSelf())
        di.bind(OneParam, t => t.toClass(OneParam, [DepA]))
        await di.init()
      ...
      it('should accept an InjectionDescriptor', async function () {
        ...
        di.bind(DepA, t => t.toSelf())
        di.bind(OneParam, t => t.toClass(OneParam, [{ key: DepA } as never]))
        await di.init()
    ...
    describe('toSelf() with injections', function () {
      ...
      it('should resolve when multiple injections are provided', async function () {
        ...
        di.bind(DepC, t => t.toSelf())
        di.bind(DepD, t => t.toSelf())
        di.bind(TwoParamSelf, t => t.toSelf([DepC, DepD]))
        await di.init()

        const svc = di.get(TwoParamSelf)

        expect(svc.c).toBeInstanceOf(DepC)
        expect(svc.d).toBeInstanceOf(DepD)
      })
      ...
      it('should resolve when a single injection is provided without wrapping array', async function () {
        ...
        di.bind(DepC, t => t.toSelf())
        di.bind(OneParamSelf, t => t.toSelf([DepC]))
        await di.init()
    ...
    describe('with options', function () {
      it('should bind dependency as transient when using .transient()', async function () {
        ...
        di.bind(Late, t => t.toSelf().lifetime(Scopes.TRANSIENT))
        await di.init()
    ...
    describe('rebinding', function () {
      it('should apply the new factory after rebind', async function () {
        ...
        di.bind(Late, t => t.toSelf())
        di.rebind(Late, t => t.toFactory(() => new Late()))
        await di.init()

        const dep = di.get(Late)

        expect(dep).toBeInstanceOf(Late)
      })
    ...
    describe('binding several functions to the same qualifier', function () {
      const kQry = token<() => string>(Symbol('queries'))

      const kQry1 = token<() => string>(Symbol('qry1'))
      const kQry2 = token<() => string>(Symbol('qry2'))
      const kQry3 = token<() => string>(Symbol('qry3'))
      const qry1 = () => 'one'
      const qry2 = () => 'two'
      const qry3 = () => 'three'
      const qry4 = () => 'four'
      ...
      it('should resolve all functions', async function () {
        ...
        di.bind(kQry1, t => t.toValue(qry1).names(kQry))
        di.bind(kQry2, t => t.toValue(qry2).names(kQry))
        di.bind(kQry3, t => t.toValue(qry3).names(kQry))
        await di.init()
    ...
    describe('scoping', function () {
      ...
      it('should bind component as request scoped', async function () {
        const di = new CaffeineIoC()
        di.bind(ReqDep, t => t.toSelf().lifetime(Scopes.REQUEST))
  ...
  describe('invalid bindings scenarios', function () {
    it('should only accept self binding with class types', function () {
      const di = new CaffeineIoC()
      expect(() =>
        di.bind(token<Record<string, unknown>>('test'), t =>
          t
    ...
    it('should only accept previously registered scopes', function () {
      const di = new CaffeineIoC()
      expect(() =>
        di.bind(token<string>('test'), t => t.toValue('value').lifetime(token<Scope>('nonexistent-scope'))),
      ).toThrow(ErrInvalidBinding)
    })
  ...
  describe('postConstruct()', function () {
    it('should call the specified method after the instance is resolved', async function () {
      ...
      const di = new CaffeineIoC()
      di.bind(SvcWithInit, t => t.toSelf().postConstruct(v => v.init()))
      await di.init()
      di.get(SvcWithInit)
  ...
  describe('lazy', function () {
    ...
    it('should not init component on bootstrap when it is configured as lazy', async function () {
      ...
      di.bind(Laziest, t => t.toSelf().lazy())
      di.bind(NonLazy, t => t.toSelf().lazy(false))
      await di.init()
  ...
  describe('$i.just()', function () {
    it('should inject a string constant', async function () {
      ...
      const di = new CaffeineIoC({ decorators: false })
      di.bind(StringParam, t => t.toSelf([$i.just('hello')]))
      await di.init()

      expect(di.get(StringParam).val).toEqual('hello')
    })
    ...
    it('should inject a numeric constant', async function () {
      ...
      const di = new CaffeineIoC({ decorators: false })
      di.bind(NumParam, t => t.toSelf([$i.just(42)]))
      await di.init()

      expect(di.get(NumParam).val).toEqual(42)
    })
    ...
    it('should inject an object by reference', async function () {
      ...
      const di = new CaffeineIoC({ decorators: false })
      di.bind(ConfigConsumer, t => t.toSelf([$i.just(config)]))
      await di.init()

      expect(di.get(ConfigConsumer).config).toBe(config)
    })
    ...
    it('should inject null', async function () {
      ...
      const di = new CaffeineIoC({ decorators: false })
      di.bind(NullParam, t => t.toSelf([$i.just(null)]))
      await di.init()

      expect(di.get(NullParam).val).toBeNull()
    })
    ...
    it('should inject undefined', async function () {
      ...
      const di = new CaffeineIoC({ decorators: false })
      di.bind(UndefinedParam, t => t.toSelf([$i.just(undefined)]))
      await di.init()

      expect(di.get(UndefinedParam).val).toBeUndefined()
    })
    ...
    it('should inject multiple constants', async function () {
      ...
      const di = new CaffeineIoC({ decorators: false })
      di.bind(MultiParam, t => t.toSelf([$i.just('localhost'), $i.just(3000)]))
      await di.init()

      expect(di.get(MultiParam).host).toEqual('localhost')
      expect(di.get(MultiParam).port).toEqual(3000)
    })
  })
  ...
  describe('aliasOf()', function () {
    it('should resolve alias to value target', async function () {
      const di = new CaffeineIoC({ decorators: false })
      const key1 = token<string>(Symbol('key1'))
      const key2 = token<string>(Symbol('key2'))
    ...
    it('should resolve alias to class target', async function () {
      ...
      const key = token<Svc>(Symbol('alias'))
      const di = new CaffeineIoC({ decorators: false })
    ...
    it('should share the same singleton instance via both keys', async function () {
      ...
      const alias = token<Svc>(Symbol('alias'))
      const di = new CaffeineIoC({ decorators: false })
    ...
    it('should support additional names on the alias', async function () {
      const key2 = token<number>(Symbol('key2'))
      const di = new CaffeineIoC({ decorators: false })

      di.bind(key2, t => t.toValue(42))
      di.bind(token<number>(Symbol('key1')), t => t.aliasOf(key2).names('named-alias'))
      await di.init()
    ...
    it('should throw ErrNoResolutionForKey when target is not registered', async function () {
      ...
      const alias = token<Unregistered>(Symbol('alias'))
      const di = new CaffeineIoC({ decorators: false })
...
describe('wrap()', function () {
  it('should return a Provider that resolves the binding on each get()', async function () {
    const di = new CaffeineIoC({ decorators: false })
    di.bind(token<{ name: string }>('svc'), t => t.toValue({ name: 'service' }))
    await di.init()

    const provider: Provider<{ name: string }> = di.wrap(token<{ name: string }>('svc'))
  ...
  it('should throw ErrNoResolutionForKey when key is not registered', function () {
    ...
    expect(() => di.wrap(token<Record<string, unknown>>('nonexistent'))).toThrow(ErrNoResolutionForKey)
  })
})

describe('wrapMany()', function () {
  it('should return a Provider<T[]> resolving all bindings for a shared name key', async function () {
    const kSvc = token<string>(Symbol('wrap-many-svc'))

    const di = new CaffeineIoC({ decorators: false })
    di.bind(token<string>('alpha'), t => t.toValue('alpha').names(kSvc))
    di.bind(token<string>('bravo'), t => t.toValue('bravo').names(kSvc))
    await di.init()
  ...
  it('should throw ErrNoResolutionForKey when no bindings exist for the key', function () {
    const di = new CaffeineIoC({ decorators: false })
    const kMissing = token<Record<string, unknown>>(Symbol('wrap-many-missing'))
  ...
  it('should return a Provider<T[]> with one element for a single binding (fast path)', async function () {
    const kSingle = token<string>(Symbol('wrap-many-single'))

    const di = new CaffeineIoC({ decorators: false })
    di.bind(token<string>('solo'), t => t.toValue('only-one').names(kSingle))
    await di.init()
...
describe('wrapBinding()', function () {
  it('should return a Provider that resolves the binding on each get()', async function () {
    const di = new CaffeineIoC({ decorators: false })
    di.bind(token<Record<string, unknown>>('svc'), t => t.toValue({ name: 'service' }))
    await di.init()

    const binding = di.getBinding(token<{ name: string }>('svc'))
...
describe('wrapBindings()', function () {
  it('should return a Provider<T[]> wrapping a single binding (fast path)', async function () {
    const di = new CaffeineIoC({ decorators: false })
    di.bind(token<string>('solo'), t => t.toValue('only-one'))
    await di.init()

    const bindings = di.getBindings(token<string>('solo'))
  ...
  it('should return a Provider<T[]> resolving all bindings (loop path)', async function () {
    const kShared = token<string>(Symbol('wrap-bindings-shared'))

    const di = new CaffeineIoC({ decorators: false })
    di.bind(token<string>('alpha'), t => t.toValue('alpha').names(kShared))
    di.bind(token<string>('bravo'), t => t.toValue('bravo').names(kShared))
    await di.init()
