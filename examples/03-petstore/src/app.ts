  ...
  const builder = createWebApplication(fastifyAdapterFactory(server), {
    ...
    .extend(OpenAPIExt, (o) =>
      o.version("3.2.0")
        .info({
          ...
          version: "1.0.0",
          description:
            "A pet adoption API, modelled on the OpenAPI 3.2 Modern Petstore specification.",
        })
    ...
    .authentication((auth) =>
      auth
        ...
        .addBasic("Basic", (o) =>
          o.realm("Petstore docs").validate((_ctx, username, password) =>
            username === docsUser && password === docsPassword
              ? new Principal(true, [
                  new Identity("Basic", true, [
                    new Claim("sub", username, "petstore"),
                  ]),
                ])
              : null,
          ),
          ...
          (o) =>
            o.clientID(githubConfig.clientID)
              ...
              .claimMapper((u) => {
                ...
                if (u.avatar_url) {
                  claims.push(
                    new Claim("avatar_url", u.avatar_url, GITHUB_ISSUER),
                  );
                }
