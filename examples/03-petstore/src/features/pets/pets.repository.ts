...
import type { CreatePetDTO, PetCollection, PetFilters, PetSearchCriteria, UpdatePetDTO } from './pet.js'
...
export class PetsRepository {
  ...
  async search(body: PetSearchCriteria): Promise<PetCollection> {
    ...
    if (c.ageRange && (c.ageRange.min !== undefined || c.ageRange.max !== undefined)) {
      where.ageMonths = {}
    ...
    const [rows, totalItems] = await Promise.all([
      this.prisma.pet.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy }),
      this.prisma.pet.count({ where }),
    ])
  ...
  async create(dto: CreatePetDTO) {
    const row = await this.prisma.pet.create({
      data: {
        ...
        photos: dto.photos ?? [],
        medicalInfo: dto.medicalInfo ? (dto.medicalInfo as Prisma.InputJsonValue) : Prisma.JsonNull,
      },
    })
    return toPetDTO(row)
  }
  ...
  async countByStatus(): Promise<Record<string, number>> {
    const groups = await this.prisma.pet.groupBy({ by: ['status'], _count: { _all: true } })
