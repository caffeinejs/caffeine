...
const specPath = fileURLToPath(new URL('../spec/openapi.petstore.yaml', import.meta.url))

let app: WebApplication
let document: OpenAPIDocument

function operation(path: string, method: string): OperationObject | undefined {
  const item = document.paths?.[path] as Record<string, OperationObject> | undefined
  return item?.[method]
}
...
function operationIds(): string[] {
  ...
  for (const item of Object.values(document.paths ?? {})) {
    ...
    for (const op of [...fixed, ...Object.values(item.additionalOperations ?? {})]) {
...
describe('the generated OpenAPI document', () => {
  ...
  it('implements exactly the operations the checked-in specification names', () => {
    ...
    for (const id of operationIds()) {
      expect(specified, `"${id}" is not an operationId in spec/openapi.petstore.yaml`).toContain(id)
    }
  ...
  it('groups operations under the controllers\' tags', () => {
    expect(document.tags?.map(t => t.name).sort())
      .toEqual(['Inventory', 'Orders', 'Pets', 'Users'])
  ...
  it('hoists the shared schemas into components', () => {
    ...
    expect(names).toEqual(expect.arrayContaining(['ApiError', 'Order', 'Pet', 'PetList', 'User']))
    expect(operation('/pets/{id}', 'get')?.responses?.['200'].content?.['application/json'].schema)
  ...
  it('never advertises the write-only password on a user response', () => {
    ...
    expect(Object.keys((user?.properties ?? {}) as object)).not.toContain('password')
  })
  ...
  it('derives the security schemes from the authentication configuration', () => {
    ...
    expect(schemes.GitHub).toMatchObject({ type: 'apiKey', in: 'cookie', name: 'petstore_gh_session' })
    expect((schemes.GitHub as { description?: string }).description)
  ...
  it('never references a security scheme that components does not define', () => {
    const defined = new Set(Object.keys(document.components?.securitySchemes ?? {}))

    for (const item of Object.values(document.paths ?? {})) {
      for (const op of Object.values(item) as OperationObject[]) {
        for (const requirement of op?.security ?? []) {
          for (const name of Object.keys(requirement)) {
            expect(defined, `"${name}" is referenced but not defined`).toContain(name)
          }
        }
      }
    }
  ...
  it('represents the QUERY verb, which 3.2 added and 3.1 cannot express', () => {
    expect(document.paths?.['/pets'].additionalOperations?.QUERY?.operationId).toBe('searchPets')
  })

  it('turns a uuid path parameter into a typed, required parameter', () => {
    const id = operation('/pets/{id}', 'get')?.parameters?.find(p => p.name === 'id')

    expect(id).toMatchObject({ in: 'path', required: true, schema: { format: 'uuid' } })
  })

  it('serves the same document as YAML', async () => {
    const res = await app.fetch('/openapi.yaml', { headers: { authorization: DOCS_AUTH } })
