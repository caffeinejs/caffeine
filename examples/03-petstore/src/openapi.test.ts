...
describe('the generated OpenAPI document', () => {
  ...
  it("groups operations under the controllers' tags", () => {
    expect(document.tags?.map(t => t.name).sort()).toEqual(['Inventory', 'Orders', 'Pets', 'Users'])
  ...
  it('hoists the shared schemas into components', () => {
    ...
    expect(operation('/pets/{id}', 'get')?.responses?.['200'].content?.['application/json'].schema).toEqual({
