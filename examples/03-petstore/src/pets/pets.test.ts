...
import type { CreatePetDTO, PetCollection, PetDTO, PetFilters, UpdatePetDTO } from './pets.schemas.js'
...
// The decorator-declared half of the application, driven through `controllerTypedClient`: the client is keyed
// by handler method name and returns the parsed body, throwing `ErrFetchFailed` on a non-2xx. `orders.test.ts`
// is the same application exercised the other way, through a router's own route types.
describe('pets feature (via @caffeinejs/testing)', () => {
  ...
  async function createPet(dto: CreatePetDTO = validPet): Promise<PetDTO> {
    return client.create(newReq().headers(sessionHeader(session)).json(dto).build())
  }
  ...
  it('refuses create without a session (write:pets)', async () => {
    await expect(client.create(newReq().json(validPet).build())).rejects.toMatchObject({ status: 401 })
  })
  ...
  it('returns 404 for an unknown id', async () => {
    await expect(client.get(new Request(newURL('/pets/:id').param('id', randomUUID()).build()))).rejects.toBeInstanceOf(
      ErrFetchFailed,
    )
    await expect(client.get(new Request(newURL('/pets/:id').param('id', randomUUID()).build()))).rejects.toMatchObject({
      status: 404,
    })
  })
  ...
  it('removes a pet by id (204) and then 404s on read', async () => {
    ...
    await expect(client.get(new Request(newURL('/pets/:id').param('id', created.id).build()))).rejects.toMatchObject({
      status: 404,
    })
  })
