    ...
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          {events.length} {events.length === 1 ? 'event' : 'events'} received — newest first
        </p>
      </div>
        ...
        {reversed.length === 0 ? (
          ...
          reversed.map((e, i) => (
            ...
            >
              ...
              <span className="break-all font-mono text-xs text-zinc-500">{JSON.stringify(e.payload)}</span>
            </div>
          ))
        )}
      </div>
