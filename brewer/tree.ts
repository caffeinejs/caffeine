...
export type Split<P extends string>
...
type ParamName<H extends string>
  = H extends '*' ? '*'
    : H extends `:${infer Rest}` ? StripMarkers<Rest>
      : never

type StripMarkers<S extends string>
  = S extends `${infer Name}(${string}` ? StripMarkers<Name>
...
type Advance<E extends AnyEntry, K extends string>
  = E extends { segs: readonly [K, ...infer T extends readonly string[]], route: infer R extends RouteContract }
    ? Entry<T, R>
    : never

/** Entries whose next segment binds a parameter. */
type ParamEntries<E extends AnyEntry>
  = E extends { segs: readonly [infer H extends string, ...string[]] }
    ? H extends `:${string}` | '*' ? E : never
    : never
 ...
 */
type ParamChild<E extends AnyEntry>
  = [ParamEntries<E>] extends [never]
    ? unknown
    : (params: ParamArg<ParamEntries<E>>) => Node<AdvanceAny<ParamEntries<E>>>

type ParamArg<E extends AnyEntry>
  = E extends { segs: readonly [infer H extends string, ...string[]], route: infer R extends RouteContract }
    ? Pick<R['params'], Extract<ParamName<H>, keyof R['params']>>
    : never

type AdvanceAny<E extends AnyEntry>
  = E extends { segs: readonly [string, ...infer T extends readonly string[]], route: infer R extends RouteContract }
    ? Entry<T, R>
    : never
...
type RawInit<R extends RouteContract>
  = CallInit<R> & ([keyof R['params']] extends [never] ? { params?: undefined } : { params: R['params'] })
