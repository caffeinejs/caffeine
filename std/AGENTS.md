...
- Health is always there. `Application.ready()` binds `ApplicationHealth` (`health/`) next to
  `ApplicationAvailability`, lazily and as a singleton, for every application kind. Indicators reach it only as
  container beans bound with `.extends(HealthIndicator)` — do not add a registration API beside that. Its budgets
