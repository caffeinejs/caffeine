...
describe('304 Not Modified', () => {
  ...
  it('GET with stale If-None-Match after cache clear → 200, new ETag', async () => {
    ...
    const server = fastify()
    const app = createWebApplication(fastifyAdapterFactory(server), { container }).extend(caching()).build()
    await app.ready()
...
describe('Cache builder & container-managed store', () => {
  ...
  it('a raw container-bound CacheStore (toClass) overrides the default and backs caching', async () => {
    let callCount = 0
  ...
  it('zero configuration → default in-process store caches without any CacheStore binding', async () => {
    ...
    const server = fastify()
    // No store configuration — CacheBuilder.bootstrap binds the default MemoryCacheStore.
    const app = createWebApplication(fastifyAdapterFactory(server)).extend(caching()).build()
    await app.ready()

    // CacheBuilder.bootstrap binds the default MemoryCacheStore under CacheStore.
  ...
  it('a container-bound ETagGenerator (kETagGenerator) drives the ETag header', async () => {
...
describe('default CacheStore', () => {
  ...
  // (caching/store.ts), and the default used to be installed over it: the configurer guards with
