...
- `.with(() => constraintsPlugin([strategy]))` — registers a custom find-my-way constraint strategy (synchronous only), exactly like wiring in CORS or Compress. No `app.constraints(...)` — a custom strategy is opt-in per application, not a built-in.

Group constraints inherit to routes that do not set the same key — for free, via the generic `config` merge (route wins, same as any other `config`/`options` key), not a dedicated mechanism. `fst({ constraints: { … } })` still works for `host` and anything the framework has no opinion about; a `constraints` key set **both** through `fst` and first-class fails when the route registers, in `constraintVaryPlugin`, rather than disagreeing silently. `constraintVaryPlugin` (installed by the adapter, always, before the `.with(...)` loop) adds every constraint header to `Vary` when any route is constrained. A constraint miss is Fastify's 404 — it does not reach `@Catch`, and no default version is invented. `ServerOwnedPaths` (probes, OIDC callbacks) never carry a constraint.
