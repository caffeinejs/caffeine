 ...
 */
export function assertAccessTokenHash(accessToken: string, atHash: string, alg: string): void {
  const expected = Buffer.from(accessTokenHash(accessToken, alg))
  const actual = Buffer.from(atHash)

  if (expected.length !== actual.length || !timingSafeEqual(expected, actual)) {
    throw new ErrOidcCallback('Cannot process OIDC callback: at_hash does not match the access token')
  }
}
