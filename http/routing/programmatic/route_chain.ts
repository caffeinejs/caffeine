...
> {
  ...
  /**
   * Selects the route on a registered route constraint, overriding the same constraint set on the group. `name`
   * must be registered — `version` always is, others through `app.constraints(...)`.
   */
  ...
  /**
   * Binds the route to an API version, selected by the `Accept-Version` request header, overriding a group
   * `.version()`. Sugar for `.constraint('version', version)`.
   */
