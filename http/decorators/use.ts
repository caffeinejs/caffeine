 ...
 *
 * The controller counterpart of `router.extend(...)`: the plugin runs in front of this controller's routes and
 * nowhere else. The factory is called once, during start-up, with the resolved configuration and the
 * container — the same arguments the application's `.extend(c => …)` gets.
 *
