 ...
 *
 * Its lifecycle contributes the live {@link ServerOptions} under {@link kServerContribution}. The listen
 * address still stops moving once the socket is bound — the application copies these options immediately
 * before binding, and that copy is what the server runs on.
 *
...
export class ServerBuilder<C = unknown> implements FeatureLifecycle {
  ...
  #resolved: ConfigSlice<ServerOptions> | undefined
   ...
   */
  config(selector: (c: ConfigHandle<C>) => ConfigLocation<ServerOptions>): this {
