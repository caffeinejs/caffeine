...
| Browser client route (`GET /settings`) | 200 `index.html` for an SPA | `@caffeinejs/static`'s `.spa(...)` — a compiled `GET /*` route, not `@Catch` |
...
Same-origin SPA + `/api` needs `.spa(root)`, not a second `@Catch`. That is routing policy: `@caffeinejs/static` derives which prefixes the server owns from the compiled routes (a controller's base path, `/api`) and serves the shell only for a browser navigation to a path outside them; `exclude` is for a path routing does not know, such as a reverse-proxied upstream. Do not return `index.html` from `@Catch(ErrHTTPNotFound)`.
