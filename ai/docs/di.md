...
- Bindings: `container.bind(InjectionToken).toClass(...)` / `.toValue(...)`. Named bindings, labels, tags, scopes (`Scopes.SINGLETON`, `Scopes.REQUEST`).
- Features are **plugins** + **builders**, not Nest `Module` / `forRootAsync`. `app.extend(kafka())` then `app.kafka(k => k.brokers(...))`.
