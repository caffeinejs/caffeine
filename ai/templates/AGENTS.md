...
- **HTTP:** `@Controller` + `@Get`/`@Post`/… + `$p`. Features via `.with(staticFiles(s => s.serve(...)))`.
- **Kafka:** `@KafkaHandler` / `@KafkaListener` / `KafkaTemplate` / `$k`. Not Nest microservices.
- **Composition:** `createWebApplication` or `createApplication`, then `.with(feature(configure))` — the feature is always a call, and a Fastify plugin factory is `.with(({ config }) => fp(instance => instance.register(fastifyCors, config.app.cors), { name: 'cors' }))`. Side-effect-import controllers and Kafka handlers.
