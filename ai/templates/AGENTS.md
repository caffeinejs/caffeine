...
- **Composition:** `createWebApplication` or `createApplication`, then `.with(feature(configure))` — the feature is always a call, and a Fastify plugin factory is `.with(({ config }) => [fastifyCors, config.app.cors])`. Side-effect-import controllers and Kafka handlers.
