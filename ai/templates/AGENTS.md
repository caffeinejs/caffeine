...
- **Composition:** `createWebApplication` or `createApplication`, then `.extend(feature(configure))` — the feature is always a call, and a Fastify plugin factory is `.plugin(c => corsPlugin(c.app.cors))`. Side-effect-import controllers and Kafka handlers.
