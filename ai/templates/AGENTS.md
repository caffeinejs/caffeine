...
- **Composition:** `createWebApplication` or `createApplication`, then `.with(feature(configure))` — the feature is always a call, and a Fastify plugin factory is `.with(c => fp(instance => instance.register(fastifyCors, c.app.cors), { name: 'cors' }))`. Side-effect-import controllers and Kafka handlers.
